...
class Client extends BaseClient
{
    ...
    private function getStory($slug, $byUuid = false)
    {
        ...
        if ($version === 'published' && $this->cache && $cachedItem = $this->cache->load($cachekey)) {
            if ($this->cacheNotFound && (int)$cachedItem->httpResponseCode === 404) {
     ...
     * @return array
     */
    private function _generateTree($parent, $items)
    {
        $tree = array();
    ...
    private function _save($response, $key, $version)
    {
        ...
        if ($this->cache &&
            ...
            (int)$response->httpResponseCode === 200) {
